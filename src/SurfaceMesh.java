...
public class SurfaceMesh {
	...
	public SurfaceMesh(MeshViewer view, String filename) {
		...
		polyhedron3D = load3D.createTriangleMesh(sharedVertex.points, sharedVertex.faceDegrees, sharedVertex.faces,
	...
	public void drawTriangle(Point_3 p, Point_3 q, Point_3 r) {
		float s = (float) this.scaleFactor;
		view.vertex((float) (p.getX().doubleValue() * s), (float) (p.getY().doubleValue() * s),
				(float) (p.getZ().doubleValue() * s));
		view.vertex((float) (q.getX().doubleValue() * s), (float) (q.getY().doubleValue() * s),
				(float) (q.getZ().doubleValue() * s));
		view.vertex((float) (r.getX().doubleValue() * s), (float) (r.getY().doubleValue() * s),
				(float) (r.getZ().doubleValue() * s));
	}
	...
	public void drawFace(Face<Point_3> f) {
		...
		// view.fill(f.tag * 200 % 256, 100 + f.tag * 100 % 256, 100 - f.tag * 150 %
		// 256, 255);
		// view.fill(255* (float) f.tag / (float) view.k, 200, 255, 255);
		int r = 100 + f.tag * 30 % 256;
		int g = 50 + f.tag * 100 % 256;
		int b = 255 - (r+g)/2; // 256 - f.tag * 30;
		view.fill(r, g, b, 255);
	...
	public void draw(int type) {
		...
		if (type == 0)
			return; // no rendering for vertices
